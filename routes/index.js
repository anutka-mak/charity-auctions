...
const {
  ...
  ProjectCategory,
  Bids,
} = require("../models/sellersProducts");
...
router.get("/shkilni-auktsiony/all/", async (req, res) => {
  ...
  const products = await Product.findAll({
    where: { projectCategoryId: 1, deletedAt: null },
    attributes: [
      "id",
      "name",
      "description",
...
router.get("/moskalevbyvchyy-motlokh/all/", async (req, res) => {
  ...
  const products = await Product.findAll({
    where: { projectCategoryId: 2, deletedAt: null },
...
router.post("/new-items", async (req, res) => {
  ...
  const photos =
    req.files.photos instanceof Array ? req.files.photos : [req.files.photos];
  ...
  try {
    ...
    if (mainPhoto) {
      ...
      const uploadPathForServer = path.join(
        __dirname,
        "../Images",
        mainPhotoFile.name
      );
